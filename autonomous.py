...
class AutonomousMode(object):
    ...
    def main_loop(self):
        ...
        try:
            print("You are about to run in autonomous mode...")
            print("Press 'start' button to activate autonomous mode")
            print("Press 'X' button to deactivate autonomous mode")
            print("Press 'options' to quit the program")

            # Load model
            model = load_model('trained_model/pilotnet_2.h5', custom_objects = {"tf": tf})
            ...
            while self.send_command:
                ...
                #Thread(target=self.display_feeds, args=(self.image, )).start()
